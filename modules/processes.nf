...
process MINIMAP2_INDEX_SECOND_ITERATION {
...
process MINIMAP2_SAM_SECOND_ITERATION {
    tag "$meta"
    memory { 4.GB * task.attempt }
    errorStrategy { task.attempt <= 5 ? "retry" : "finish" }
    maxRetries 5
...
process SAM_SORT_AND_INDEX_SECOND_ITERATION {
    tag "$meta"
...
process EXTRACT_MICROBIAL_READS_SECOND_ITERATION {
    publishDir "${params.output_dir}", mode:'copy'
    tag "extract microbial DNA from $sample_id"
...
process BAM_STATISTICS_SECOND_ITERATION {
   // publishDir "${params.output_dir}", mode:'copy'
    tag "$sample_id"
...
process COMBINE_BAM_STATISTICS_SECOND_ITERATION {
    publishDir "${params.output_dir}", mode:'copy'
    tag { 'combine bam statistics files'} 
    ...
    output:
    path("combined_bam_stats_2nd_iteration.txt"), emit: bam_comb_stats_ch
